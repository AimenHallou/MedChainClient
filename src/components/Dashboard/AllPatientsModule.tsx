...
const AllPatientsModule = () => {
  ...
  const {
    ...
    queryFn: () =>
      getPatients(pagination.page, pagination.limit, filter, sortBy, sortOrder),
  });
        ...
        {data && data.totalCount > 0 && (
          <Pagination>
            <PaginationContent>
              ...
              {Array.from({
                length: Math.ceil(data.totalCount / pagination.limit),
              }).map((_, i) => (
                <PaginationItem key={i}>
                  <PaginationLink
                    className={cn(
                      "select-none cursor-pointer",
                      pagination.page === i && "bg-secondary text-white"
                    )}
                    onClick={() => {
              ...
              <PaginationItem>
                <PaginationNext
                  ...
                  onClick={() => {
                    if (
                      pagination.page <
                      Math.floor(data.totalCount / pagination.limit)
                    ) {
                      setPagination((prev) => ({
                        ...prev,
                        page: prev.page + 1,
